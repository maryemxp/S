...
// --- START FROM CLEAN SLATE ON LOAD ---
...
function initAuth() {
    ...
    authForm.addEventListener("submit", (e) => {
        ...
        if(userIn) {
            ...
            // Now load user specific progression safely
            loadFromLocalStorage(state.user);
        }
    });
}

// --- NAVIGATION ENGINE (WITH BACK BUTTON) ---
...
function initAICore() {
    ...
    document.getElementById("btn-calc-calories").addEventListener("click", () => {
        ...
        out.innerHTML = `<strong>🤖 AI Core Analysis Complete:</strong><br>
                         ...
                         • Composition: 45% Carbs, 30% Healthy Fats, 25% High Proteins.<br>
                         • Ingredients detected: Avocado, Artisan Sourdough, Poached egg.`;
    });
    ...
    document.getElementById("btn-get-style").addEventListener("click", () => {
        ...
        if(temp <= 15) {
            ...
            proposal = "<b>Top:</b> Ultra-breathable cream white linen shirt.<br><b>Pants:</b> Soft light-wash relaxed denim jeans.";
        }
...
function renderVirtualRoom() {
    ...
    if(state.streak === 0) {
        atmosphere.style.backgroundColor = "rgba(0, 0, 0, 0.45)"; 
    } else if (state.streak >= 7) {
        atmosphere.style.backgroundColor = "rgba(0, 240, 255, 0.08)"; 
    } else {
        atmosphere.style.backgroundColor = "rgba(112, 0, 255, 0.05)"; 
    }
}
...
function startDailyLesson() {
    ...
    currentLesson.words.forEach(wObj => {
        ...
        card.innerHTML = `
            <h4>${wObj.w}</h4>
            <div class="ipa-pron">${wObj.p} <span style="cursor:pointer;" onclick="alert('🔊 Playing audio simulation')">🎵</span></div>
    ...
    document.getElementById("btn-go-quiz").classList.remove("hidden");
}
